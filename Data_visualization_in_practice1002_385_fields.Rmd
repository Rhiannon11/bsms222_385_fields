...
2^2,2^3,2^4,... or 10^2,10^3,… are easy to compute in our heads, the same is not true for  e2,
e3,… , so the scale is not intuitive or easy to interpret.
...
is an integer and between -10 and 10, so we prefer to have smaller integers in the scale. Another consequence of a limited range is that choosing the binwidth is more challenging. With log base 2, we know that a binwidth of 1 will translate to a bin with range x to 2x.
