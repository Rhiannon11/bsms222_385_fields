---
title: rbasic_20190909_385_fields
...
avg <- function(x, arithmetic = TRUE){
  n <- length(x)
  infelse(arithmetic, sum(x)/n, prod(x)^(1/n))
}
```
...
7. Write a function compute_s_n that for any given n computes the sum Sn=1^2+2^2+3^2+…n2. Report the value of the sum when n=10.

```{r}
n <- 10
n2 <- 10*2
compute_s_n <-(1^2+2^2+3^2+n2)
compute_s_n
```
...
```{r}
?sapply
sapply(x, 25, simplify=TRUE, USE.NAMES=TRUE)
```
