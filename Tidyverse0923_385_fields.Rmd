...
```{r}
ref <- NHANES %>% 
  filter(syatolic blood pressure == "BPSysAve") %>%
  summarize(average == mean(NHANES), standard deviation == sd(NHANES))
```
...
```{r}
murders_tibble <- class(as_tibble(murders))
murders_tibble
```
